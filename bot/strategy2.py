"""
Strategy 2: Momentum Pro — Smarter entries, fewer losses
    ...
    async def _tick(self):
        ...
        for cid, ws in list(self._windows.items()):
            ...
            if ws.open_price is None:
                ...
                if now >= ready:
                    ws.open_price = btc
                    log.info("S2 Pro: baseline $%.2f for %s", btc, ws.market.question[:40])
            ...
            if spike_dir != window_dir:
                ...
                log.info("S2 Pro REJECTED trend: $%+.0f spike but $%+.0f from open", spike, window_move)
                continue
            ...
            if vol < MIN_VOLUME_BTC:
                ...
                log.info("S2 Pro REJECTED volume: %.1f BTC < %.1f minimum", vol, MIN_VOLUME_BTC)
                continue
            ...
            log.info(
                "S2 Pro SIGNAL: $%+.0f in %.0fs | vol=%.1fBTC | range=$%.0f | thresh=$%.0f | BTC $%+.0f from open → %s | %s",
    ...
    async def _discover(self):
        ...
        for mkt in markets:
            if mkt.condition_id not in self._windows:
                self._windows[mkt.condition_id] = S2Window(market=mkt)
    ...
    async def _check_exits(self):
        ...
        for pos in self._open_positions:
            ...
            if btc_now and ws and ws.open_price and gain < 0:
                ...
                if wrong:
                    log.warning("S2 Pro REVERSAL: %s flipped → selling", pos.side)
            ...
            if not pos.moonbag_mode and gain >= MOONBAG_PCT:
                ...
                log.info("S2 Pro MOONBAG: %s +%.1f%%", pos.side, gain)
            ...
            if should_sell:
                log.info("S2 Pro EXIT [%s]: %s gain=%.1f%%", reason, pos.side, gain)
                sold = await self.poly.sell(pos)
                if sold:
    ...
    def _record_exit(self, pos, loss: bool, reason: str):
        ...
        if loss:
            ...
            if self._consecutive_losses >= COOLDOWN_LOSSES:
                ...
                log.warning(
                    "S2 Pro COOLDOWN: %d consecutive losses → pausing %ds",
