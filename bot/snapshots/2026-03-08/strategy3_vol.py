            ...
            if remaining <= ANALYSIS_START and remaining > BUY_WINDOW_END:
                ...
                if (remaining <= BUY_WINDOW_START and
                    ...
                    if buy_side:
                        if self._skip_next:
                            log.info(
                                "  STREAK BREAKER: skipping after 3 consecutive losses | %s %s @ %.2f",
                            ...
                            phantom = S3Position(
                                ...
                                vol_snapshot={
                                    "remaining": remaining,
                                    "filtered": True,
                                    "filter_reasons": ["streak_breaker:3_consecutive_losses"],
                                    "skip_reason": "",
                                },
                            )
                            self._phantoms.append(phantom)
                            self.stats.filtered_out += 1
                            self.stats.last_action = f"STREAK BREAK SKIP {buy_side}"
